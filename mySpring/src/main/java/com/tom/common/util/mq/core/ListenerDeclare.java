...
import com.tom.common.util.mq.listener.BaseChannelListener;
import com.tom.common.util.mq.listener.BaseListener;
...
public class ListenerDeclare {
	...
	public boolean declareLisener(ThreadPoolTaskExecutor poolTaskScheduler){
		...
		ListenerStatus status = ListenerFactory.instantiation().keepListener(connectionFactory, container);
		if(status.getCode()==0){
			container.start();
			return true;
		}else{
	...
	public boolean changeLisener(int consumerSize){
		...
		SimpleMessageListenerContainer listenerContainer = ListenerFactory.instantiation().getListener(key);
